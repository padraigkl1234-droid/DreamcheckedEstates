...
// Zone-level drill-down for the Ballroom — a 2.5D isometric floor plan (see
// components/ZoneFloorPlan.tsx) with clickable safety-equipment pins, reached
// from the Site Map by clicking into the Ballroom zone. Each pin is a
// SiteAsset: a real Firestore doc with its own compliance due-date, so this
// is a visual index into live data rather than a static picture.
...
export default function BallroomFloorPlanPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      query(collection(db, 'assets'), where('teamId', '==', teamId), where('zone', '==', BALLROOM_PLAN.zone)),
      (snap) => {
        setError(null);
  ...
  const seedStarters = async () => {
    ...
    setError(null);
    try {
      for (const starter of BALLROOM_PLAN.starterAssets) {
        const id = genId();
        await setDoc(doc(db, 'assets', id), {
          id,
          type: starter.type,
  ...
  return chrome(
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-neutral-100 sm:text-3xl">Ballroom</h1>
      ...
      <ZoneFloorPlan plan={BALLROOM_PLAN} assets={assets} canEdit={canEdit} onCreate={create} onUpdate={update} onDelete={remove} />
    </div>
  );
}
