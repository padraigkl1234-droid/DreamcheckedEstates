...
import { InvictusOrb, type OrbState } from '@/components/InvictusOrb';
import { InvictusClock } from '@/components/InvictusClock';
import { useInvictusVoice } from '@/components/useInvictusVoice';
...
export default function InvictusPage() {
  ...
  const orbState: OrbState =
    ...
    voice.state === 'listening' || isThinking ? 'listening' : 'idle';

  const orbAmplitude =
    voice.state === 'listening' ? voice.amplitude :
    voice.state === 'speaking' ? voice.amplitude :
    isThinking ? thinkingAmp : 0;

  const handleMicToggle = () => {
    if (voice.state === 'listening') { voice.stopListening(); return; }
    if (voice.state === 'speaking') { voice.cancelSpeech(); return; }
