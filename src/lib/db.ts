...
const DATA_DIR = process.env.VERCEL
  ? path.join('/tmp', 'invictus-data')
  : path.join(process.cwd(), 'data');
const DB_PATH = path.join(DATA_DIR, 'invictus.db');
...
export function getDb(): Database.Database {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS workout_split (weekday INTEGER PRIMARY KEY, label TEXT NOT NULL, focus TEXT NOT NULL, exercises TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS nutrition_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, time TEXT NOT NULL, food_name TEXT NOT NULL, calories REAL NOT NULL DEFAULT 0, protein_g REAL NOT NULL DEFAULT 0, carbs_g REAL NOT NULL DEFAULT 0, fat_g REAL NOT NULL DEFAULT 0, notes TEXT, created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS hydration_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, time TEXT NOT NULL, amount_ml REAL NOT NULL, created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS workout_sessions (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, split_label TEXT, notes TEXT, created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS workout_sets (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id INTEGER NOT NULL, exercise_name TEXT NOT NULL, set_number INTEGER NOT NULL, reps INTEGER, weight_kg REAL, rpe REAL, created_at TEXT NOT NULL, FOREIGN KEY (session_id) REFERENCES workout_sessions(id) ON DELETE CASCADE);
    CREATE TABLE IF NOT EXISTS body_metrics (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, weight_kg REAL, body_fat_pct REAL, resting_hr REAL, sleep_hours REAL, notes TEXT, created_at TEXT NOT NULL);
  ...
  if (splitCount.c === 0) {
    const ins = db.prepare('INSERT INTO workout_split (weekday, label, focus, exercises) VALUES (?, ?, ?, ?)');
    db.transaction(() => { for (const d of DEFAULT_SPLIT) ins.run(d.weekday, d.label, d.focus, JSON.stringify(d.exercises)); })();
  }
